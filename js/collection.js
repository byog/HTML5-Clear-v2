 ...
 * @class
 * @hideconstructor
 * @classdesc The C.Collection base object, inherited by C.ListCollection and C.TodoCollection
 * @extends C
 * @name Collection
 */
C.Collection = (function (raf) {
     ...
     * @var {Number}
     * @default 0.45
     * @name Collection~dragElasticity
     */
    var dragElasticity = 0.45,
         ...
         * @var {Number}
         * @default 16
         * @name Collection~speedMultiplier
         */
        speedMultiplier = 16,
        /**
         * @inner
         * @var {Number}
         * @default 35
         * @name Collection~maxSpeed
         */
        maxSpeed = 35,
        /**
         * the min distance from target an animation loop chain should reach before ending
         * @inner
         * @var {Number}
         * @default 0.5
         ...
         * @var {Number}
         * @default 4.5
         * @name Collection~sortMoveSpeed
         */
        sortMoveSpeed = 4.5

    /**
     * used to record position before edit focus
     * @inner
     * @var {Number}
     * @default 0
     * @name Collection.beforeEditPosition
     */
    var beforeEditPosition = 0
    ...
    var collection = {
            ...
            if (this.y > 0) {
                ...
                if (this.y <= C.ITEM_HEIGHT) {
                    if (this.pastPullDownThreshold) {
                        this.pastPullDownThreshold = false
                        this.topDummyText.text(
                            'Pull to Create ' + this.itemTypeText
                        )
                    }
                    var pct = this.y / C.ITEM_HEIGHT
                    var r = Math.max(0, (1 - pct) * 90)
                    this.topDummySliderStyle[C.client.transformProperty] =
                        'rotateX(' + r + 'deg)'
                    ...
                    if (!this.pastPullDownThreshold) {
                        ...
                        this.topDummyText.text(
                            'Release to Create ' + this.itemTypeText
                        )
                    }
                }
            } else {
        ...
        /**
         * the default on drag end, bounce the thing back into bounds, only gets called when no action is triggered
         * @function
         * @static
         * @param {*} speed
         */
        onDragEnd: function (speed) {
            ...
            speed = Math.max(
                -maxSpeed,
                Math.min(maxSpeed, speed * speedMultiplier)
            )
            ...
            function loop() {
                ...
                if (col.y < col.upperBound - diff) {
                    ...
                    speed *= 0.85 // apply additional friction
                    if (col.y < col.upperBound - diff) {
                        raf(loop)
                        render()
                    } else {
            ...
            function render() {
                ...
                if (col.y >= 0) {
                    ...
                    var r = Math.max(0, (1 - pct) * 90)
                    col.topDummySliderStyle[C.client.transformProperty] =
                        'rotateX(' + r + 'deg)'
        ...
        onEditStart: function (at, noRemember) {
            ...
            setTimeout(function () {
                ...
                if (noRemember) {
                    t.el.removeClass('drag').addClass('ease-out')
                    t.moveY(0)
        ...
        createItemAtBottom: function () {
            ...
            setTimeout(function () {
                newItem.el.find('.slider')[0].style[
        ...
        createItemInBetween: function () {
            ...
            newItem.el
                .addClass('drag')
                .css('opacity', 0.01) // hack hack hack...
                .find('.field')
                .show()
                .focus() // trigger keyboard in advance

 ...
 * @class
 * @hideconstructor
 * @classdesc The C.Item base object, inherited by C.ListItem and C.TodoItem
 * @extends C
 * @name Item
 */
C.Item = (function (raf) {
     ...
     * @inner
     * @var {Number}
     * @name Item.upperSortMoveThreshold
     */
    var upperSortMoveThreshold = C.ITEM_HEIGHT * 1.5,
        /**
         * @inner
         * @var {Number}
         * @name Item.lowerSortMoveThreshold
         */
    ...
    var item = {
        ...
        init: function (data) {
            ...
            this.el.append(this.check).append(this.cross)
        ...
        moveX: function (x) {
            this.x = x
            this.sliderStyle[C.client.transformProperty] =
        ...
        moveCheck: function (x) {
            this.checkX = x
            this.checkStyle[C.client.transformProperty] =
                'translate3d(' + x + 'px,0px,0px)'
        },
        ...
        moveCross: function (x) {
            this.crossX = x
            this.crossStyle[C.client.transformProperty] =
                'translate3d(' + x + 'px,0px,0px)'
        },
        ...
        onSortStart: function () {
            this.el.addClass('sorting-trans').addClass('sorting')
        },
        ...
        onSortMove: function (dy) {
            ...
            if (cy < 0 && ay < upperSortMoveThreshold && dy < 3) {
                ...
                ay > C.client.height - lowerSortMoveThreshold &&
                dy > -3
            ) {
         ...
         */
        checkSwap: function () {
            var currentAt = Math.min(
                    this.collection.items.length - 1,
                    ~~((this.y + C.ITEM_HEIGHT / 2) / C.ITEM_HEIGHT)
                ),
                origin = this.data.order

            if (currentAt != origin) {
                var targets = this.collection.getItemsBetween(
                        origin,
                        currentAt
                    ),
                    i = targets.length,
        ...
        onEditDone: function () {
            ...
            t.collection.onEditDone(function () {
                ...
                if (!val) {
                    ...
                    t.field.hide()
                    t.title.show().find('.text').text(val)
                    t.data.title = val
                    C.db.save()
                }
            })
